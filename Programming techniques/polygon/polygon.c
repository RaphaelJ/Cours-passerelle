...
int point_cmp(void *v_pt1, void *v_pt2, void* v_first)
{
    ...
    // of CPU cycles needed by the comparison (from ~40 to ~5 cycles on a
    // new Core processor).
    if (y1 * x2 < y2 * x1)
        return -1;
    else
    ...
    // same position on both two types, which is not defined in the C language
    // specification.
}